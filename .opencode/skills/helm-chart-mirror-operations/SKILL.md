...
- Pages catalog snapshot: `assets/catalog.json`, regenerated by
...
- `scripts/sub2api.sh update` requires this exact root README line:
  `  - **sub2api** chart version:`.
- `SOURCE_CHART_OCI` (`oci://ghcr.io/ben-wangz/k8s-at-home-charts/sub2api`) is
  the chart template only. It is not the application release source and not
  the publication target. Application versions come from
  `Wei-Shaw/sub2api` GitHub Releases. The published artifact is always
...
- `.github/workflows/update-sub2api.yml` runs daily or by manual dispatch. It
  resolves the latest stable release, pulls the upstream OCI chart template,
  increments the mirror chart patch version, updates the image tag and digest,
  rebuilds the package, regenerates the index, and opens a PR from
  `automation/sub2api-update`. It does not refresh `assets/catalog.json`.
- `.github/workflows/mirror-ci.yml` fails when `assets/catalog.json` drifts
  from `charts/index.yaml`. That is separate from `Sub2API chart CI`.
...
When reporting live GitOps, ArgoCD, or Pod state, load
`sub2api-72602-operations` before the first cluster read. The application
release comes from the upstream GitHub Release and resolved image manifest;
never infer it from a chart `appVersion`.

Do not dispatch workflows, create branches or PRs, merge, publish, or change
the 72602 cluster in check mode. If the latest `Update Sub2API chart` run
says `already current`, that is a no-op after the current application is
already packaged; look up the last PR that added a `.tgz` when reporting how
the current chart was published.
...
2. On `AaronYang0628/helm-chart-mirror` `main`, confirm the exact README
   marker `  - **sub2api** chart version:` still exists. If a README rewrite
   removed it, restore the current published chart/application tuple through
   a branch and PR first. Do not dispatch while that marker is missing.
3. Dispatch `Update Sub2API chart` on `main` and wait for a completed result.
   A scheduled `already current` result is a no-op, not the last successful
   package publication; find the last PR that actually added the current
   `.tgz` when describing the current package.
4. Inspect the generated `automation/sub2api-update` PR, package, index,
   README, script, workflow diff, and CI. Distinguish `Update Sub2API chart`
   CI from `Mirror consistency CI`. Run `bash scripts/sub2api.sh verify` on
   the proposed package and `git diff --check`.
5. If `Mirror consistency CI` fails because `assets/catalog.json` drifted from
   `charts/index.yaml`, regenerate it with
   `python3 scripts/generate-catalog-json.py` on that same PR, commit only the
   generated correction, and wait for both `Update Sub2API chart` CI and
   `Mirror consistency CI` to succeed before merge.
6. Merge only the reviewed PR after both required CI jobs succeed. Preserve
   previous chart packages and never overwrite an existing chart version.
7. Wait for successful `Publish Sub2API chart` and Pages workflows on the
   merge commit. Accepted or pending workflows are not successful. The
   publish workflow must anonymously pull the OCI chart back with matching
   package SHA-256.
8. Independently repeat the anonymous Helm pull and compare package SHA-256
   before reporting the chart as published.
9. Report the Git/PR/workflow URLs or SHAs, final chart/application/digest,
...
2. Confirm the root README still contains
   `  - **sub2api** chart version:` and that `assets/catalog.json` matches
   the latest `charts/index.yaml` entry.
3. Inspect the package with `helm show chart`, `helm show values`, and the
   index entry in `charts/index.yaml`.
4. Run `bash scripts/sub2api.sh verify` before treating a package as valid.
...
5. For GitHub Actions work, inspect the workflow run and PR rather than
...
- Do not dispatch `Update Sub2API chart` while the root README marker is
  missing. Pulling the chart template from `ben-wangz` is expected; do not
  treat that pull as publishing to the wrong repository.
- After the update PR adds a new `charts/index.yaml` entry, regenerate
  `assets/catalog.json` before merge if Mirror consistency CI requires it.
